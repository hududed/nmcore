...
export const stripeWebhook = onRequest(
  {
    cors: true,
    secrets: [stripeSecretKey, stripeWebhookSecret, sendgridApiKey],
  },
  async (req, res) => {
    // Initialize Stripe
    ...
    try {
      ...
      switch (event.type) {
        case 'checkout.session.completed': {
          ...
          const items: Item[] = sessionWithCustomer.metadata?.items
          ...
          for (const item of items) {
            ...
            let productSizes: any[] = productData.productSizes;
