...
<nav class="bg-white border-b border-gray-200 py-2 lg:py-4 w-full fixed top-0 left-0 z-50">
  <div class="container flex justify-between items-center">
    <div class="flex items-center space-x-4">
      <div class="md:hidden">
        <!-- Mobile menu button -->
        <Button variant="outline" class="text-gray-700 hover:text-gray-900 focus:outline-none" onclick={toggleMobileMenu}>
          <svg class="h-6 w-6" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h16"></path>
          </svg>
        </Button>
      </div>
      <img src="/images/nmcore_logo.jpg" alt="NMCore BLOOM Logo" class="w-36 lg:w-48 hidden md:block" />
    </div>
    ...
    <div class="flex items-center space-x-4">
      <Button variant="outline" class="flex items-center rounded-full bg-sky-600 px-4 py-2 text-white hover:bg-sky-700" onclick={toggleCart}>
